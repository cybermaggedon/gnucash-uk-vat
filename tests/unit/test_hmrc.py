...
def create_aiohttp_mock(mock_response, status=200):
    ...
    # Create mock session with nullcontext for async context manager support
    mock_session = Mock(
        spec=aiohttp.ClientSession,
        **{
            'get.return_value': contextlib.nullcontext(mock_resp),
            'post.return_value': contextlib.nullcontext(mock_resp)
        }
    )
    
    # Return a patch target that creates the session with nullcontext
    return contextlib.nullcontext(mock_session)
...
class TestVat:
    ...
    def test_build_fraud_headers(self, vat_client):
        ...
        with patch("gnucash_uk_vat.hmrc.config.get_gateway_ip", autospec=True, spec_set=True, return_value="192.168.1.100"):
    ...
    async def test_get_auth_coro(self, vat_client):
        ...
        with patch('aiohttp.ClientSession', return_value=create_aiohttp_mock(mock_response)):
    ...
    async def test_test_fraud_headers_error(self, vat_client):
        ...
        with patch('aiohttp.ClientSession', return_value=create_aiohttp_mock(mock_response, 400)):
            with pytest.raises(RuntimeError) as exc_info:
    ...
    async def test_submit_vat_return(self, vat_client):
        ...
        with patch('aiohttp.ClientSession', return_value=create_aiohttp_mock(mock_response, 201)):
